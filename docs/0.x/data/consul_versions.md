...
`hcp.consul_versions.withTimeouts` constructs a mixin object that can be merged into the `consul_versions`
...
`hcp.consul_versions.withTimeoutsMixin` constructs a mixin object that can be merged into the `consul_versions`
Terraform data source block to set or update the timeouts field.
